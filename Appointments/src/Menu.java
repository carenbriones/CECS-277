...
public class Menu {
	...
	public static void main(String[] args) {
		...
		do{
			...
			case 1:
				...
				Appointment appt = new Appointment(apptDate, startTime, endTime, des);
			...
			case 2:
				...
				if(apptList.size() == 0){
					...
					for (int i = 0; i < apptList.size(); i++){
						...
						if(compare.getDate().getYear() == removeApptDate.getYear()){
							if(compare.getDate().getMonth() == removeApptDate.getMonth()){
								if(compare.getDate().getDay() == removeApptDate.getDay()){
									if(compare.getStartTime().getHour() == startTime.getHour()){
										if(compare.getStartTime().getMinute() == startTime.getMinute()){
											match = i;
										}
									}
								}
			...
			case 3:
				...
				Date dateOfAppointments = new Date(year, month, day);
				
				//Counter increases if there is a match or not
				int counter = 0;
				for (int i = 0; i < apptList.size(); i++){
					if(apptList.get(i).getDate().getYear() == dateOfAppointments.getYear()){
						if(apptList.get(i).getDate().getMonth() == dateOfAppointments.getMonth()){
							if(apptList.get(i).getDate().getDay() == dateOfAppointments.getDay()){
	...
	public static Date checkDate(){
		...
		Date apptDate = new Date(year, month, day);
